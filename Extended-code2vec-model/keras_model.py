...
class Code2VecModel(Code2VecModelBase):
    ...
    def _create_keras_model(self):
        ...
        for rep in self.config.CODE_REPRESENTATIONS:
            ...
            context_valid_mask = Input((self.config.MAX_CONTEXTS[rep],))
            ...
            input_nodes[rep + '_path_input'] = path_input
            input_nodes[rep + '_path_target_token_input'] = path_target_token_input
            input_nodes[rep + '_context_valid_mask'] = context_valid_mask
    ...
    def _create_data_reader(self, estimator_action: EstimatorAction, repeat_endlessly: bool = False):
        return PathContextReader(
            vocabs=self.vocabs,
            config=self.config,
            model_input_tensors_former=_KerasModelInputTensorsFormer(self.config, estimator_action=estimator_action),
...
class ModelEvaluationCallback(MultiBatchCallback):
    ...
    def on_multi_batch_end(self, batch, logs, multi_batch_elapsed):
        self.perform_evaluation(multi_batch_elapsed)
...
class _KerasModelInputTensorsFormer(ModelInputTensorsFormer):
    ...
    def __init__(self, config, estimator_action: EstimatorAction):
        self.config = config
    ...
    def to_model_input_form(self, input_tensors: ReaderInputTensors):
        ...
        if self.estimator_action.is_predict:
            for rep in self.config.CODE_REPRESENTATIONS:
                inputs += (input_tensors.input_dict[rep + '_path_source_token_strings'], input_tensors.input_dict[rep + '_path_strings'],
                        input_tensors.input_dict[rep + '_path_target_token_strings'])
    ...
    def from_model_input_form(self, input_row) -> ReaderInputTensors:
        ...
        for index, rep in enumerate(self.config.CODE_REPRESENTATIONS):
            input_dict[rep + '_path_source_token_indices'] = inputs[index * 4]
            input_dict[rep + '_path_indices'] = inputs[(index * 4) + 1]
            input_dict[rep + '_path_target_token_indices'] = inputs[(index * 4) + 2]
            input_dict[rep + '_context_valid_mask'] = inputs[(index * 4) + 3]

        for index, rep in enumerate(self.config.CODE_REPRESENTATIONS):
            if self.estimator_action.is_predict:
                input_dict[rep + '_path_source_token_strings'] = inputs[ (len(self.config.CODE_REPRESENTATIONS) * 4) + (index * 3) ]
                input_dict[rep + '_path_strings'] = inputs[ (len(self.config.CODE_REPRESENTATIONS) * 4) + (index * 3) + 1 ]
                input_dict[rep + '_path_target_token_strings'] = inputs[ (len(self.config.CODE_REPRESENTATIONS) * 4) + (index * 3) + 2 ]
            else:
                input_dict[rep + '_path_source_token_strings'] = None
                input_dict[rep + '_path_strings'] = None
                input_dict[rep + '_path_target_token_strings'] = None
